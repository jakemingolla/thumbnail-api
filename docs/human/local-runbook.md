...
Compose LocalStack Lambda knobs live in `docker-compose.yml` (ESM poller, `LAMBDA_LIMITS_CONCURRENT_EXECUTIONS=20`, `LAMBDA_KEEPALIVE_MS=30000`). Changing them requires recreating LocalStack: `just localstack-down` then `just localstack-up` — `just apply` is not enough.
